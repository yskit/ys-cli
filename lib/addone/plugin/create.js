...
module.exports = name => {
  new dbo().until(async thread => {
    ...
    const pluginCommandFilePath = path.resolve(pluginDir, 'command.plugin.js');
    ...
    fs.writeFileSync(pluginCommandFilePath, template['plugin:/command.plugin.js'], 'utf8');
    FileList.push('command.plugin.js');
